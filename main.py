...
# ── Cookie file path ──────────────────────────────────────────────────────────
# cookies.txt ko same folder mein rakho jahan main.py hai
...
# ══════════════════════════════════════════════════════════════════════════════
#  SSRF PROTECTION — private / loopback IPs block
# ══════════════════════════════════════════════════════════════════════════════
def _is_safe_url(url: str) -> bool:
    ...
    try:
        ...
        if scheme not in ("http", "https"):
            return False                          # file://, ftp://, etc. block

        hostname = parsed.hostname
        if not hostname:
            return False

        # Try to parse as IP directly (handles 127.0.0.1, ::1, etc.)
        try:
            ip = ipaddress.ip_address(hostname)
            if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_reserved:
                return False
        except ValueError:
            pass  # It's a hostname — allow (DNS resolution checked at OS level)
...
def _base_opts(extra: dict | None = None) -> dict:
    """Build yt-dlp base options. Injects cookies for YouTube."""
    opts = {
        "quiet": True,
        "no_warnings": True,
        "noplaylist": True,
...
def _maybe_add_cookies(opts: dict, url: str) -> dict:
    """
    YouTube ke liye cookies.txt inject karo.
    File exist nahi karta toh silently skip karo.
    """
...
def _blocking_download(url: str, download_dir: str, quality: str):
    ...
    try:
        ...
        # Bot-detection specific message
...
# ══════════════════════════════════════════════════════════════════════════════
#  HTML UI  (same as v3.0 — unchanged)
# ══════════════════════════════════════════════════════════════════════════════
...
<main class="main">
  <div class="hero">
    <div class="hero-badge">✦ Universal Downloader v3.1</div>
...
<footer>Built by <span>@MANDAL4482</span> &nbsp;·&nbsp; yt-dlp + ffmpeg + FastAPI &nbsp;·&nbsp; v3.1</footer>
...
async function fetchInfo(){
  ...
  try{
    ...
    data.formats.forEach(f=>{
      const btn=document.createElement('button');btn.className='qbtn'+(f.quality==='audio'?' audio':'');btn.dataset.quality=f.quality;btn.dataset.label=f.label;
      const sz=fmtSize(f.filesize);
      btn.innerHTML=`<div class="qbtn-label">${f.quality==='audio'?'🎵':''} ${f.label}</div>${sz?`<span class="qbtn-size">${sz}</span>`:''}`;
      btn.onclick=()=>startDownload(f.quality,btn,f.label);grid.appendChild(btn)
    });
    ...
    setStatus('❌ &nbsp;Network error: '+e.message,'err');showToast('❌ Network error: '+e.message,'err');
  }finally{
    btn.disabled=false;btn.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 21l-4.35-4.35M17 11A6 6 0 1 1 5 11a6 6 0 0 1 12 0z"/></svg> Fetch';
  }
}
...
async function startDownload(quality,btnEl,label){
  ...
  try{
    ...
    clearInterval(timer);setProg(0,false);setStatus('❌ &nbsp;'+e.message,'err');showToast('❌ '+e.message,'err',4000);
  }
...
function demoFill(p){
  ...
  const samples={yt:'https://www.youtube.com/watch?v=dQw4w9WgXcQ',ig:'https://www.instagram.com/p/example/',tt:'https://www.tiktok.com/@user/video/example',tw:'https://twitter.com/i/status/example',fb:'https://www.facebook.com/watch?v=example'};
  el.value=samples[p]||'';el.focus();
}
...
async function pingServer(){
  try{
    ...
    if(res.ok){document.getElementById('pingDot').className='ping-dot online';document.getElementById('spPing').textContent=new Date().toLocaleTimeString();document.getElementById('spStatus').textContent='🟢 Online';document.getElementById('statStatus').textContent='Online'}
    else throw new Error();
  }catch{document.getElementById('pingDot').className='ping-dot offline';document.getElementById('spStatus').textContent='🔴 Offline';document.getElementById('statStatus').textContent='Offline'}
}
async function fetchStats(){
  try{
    const res=await fetch('/stats',{cache:'no-store'});const data=await res.json();
    const upSec=data.uptime_seconds||0;const days=Math.floor(upSec/86400),hrs=Math.floor((upSec%86400)/3600),mins=Math.floor((upSec%3600)/60);
    const uptimeStr=days>0?`${days}d ${hrs}h`:hrs>0?`${hrs}h ${mins}m`:`${mins}m`;
    document.getElementById('spUptime').textContent=uptimeStr;document.getElementById('spDl').textContent=data.total_downloads||0;document.getElementById('statUptime').textContent=uptimeStr;document.getElementById('statDl').textContent=data.total_downloads||0;document.getElementById('topbarUptimeVal').textContent=uptimeStr;
  }catch(e){}
}
async function fetchHealth(){
  try{
    const res=await fetch('/health',{cache:'no-store'});const data=await res.json();
    const ck=document.getElementById('spCookies');
    if(ck)ck.textContent=data.youtube_cookies==='loaded'?'✅ Loaded':'⚠️ Missing';
  }catch(e){}
}
